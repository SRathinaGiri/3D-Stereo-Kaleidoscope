...
function createWordObject(text) {
    ...
    for (const char of text) {
        ...
        const textGeometry = new TextGeometry(char, { font, size: 1.0, depth: 0.2 });
        textGeometry.computeBoundingBox();
        const mesh = new THREE.Mesh(textGeometry, buildMaterial());
        mesh.position.x = currentX;
        wordGroup.add(mesh);
        const bbox = textGeometry.boundingBox;
        const charWidth = bbox ? bbox.max.x - bbox.min.x : 0.6;
        currentX += charWidth + charSpacing;
    }
    return wordGroup;
}
function populateSliceWithText(sliceGroup, effect) { if (!font) return; const wordCount = parseInt(objectCountInput.value, 10) || 10; const placedObjects = []; const rotStep = new THREE.Euler(THREE.MathUtils.degToRad(parseFloat(stepRotateX.value) || 0), THREE.MathUtils.degToRad(parseFloat(stepRotateY.value) || 0), THREE.MathUtils.degToRad(parseFloat(stepRotateZ.value) || 0)); const scaleStepVal = parseFloat(stepScale.value) || 1; for (let i = 0; i < wordCount; i++) { for (let attempt = 0; attempt < 50; attempt++) { const wordObject = createWordObject(textInput.value); if (!wordObject) continue; new THREE.Box3().setFromObject(wordObject).getBoundingSphere(wordObject.boundingSphere = new THREE.Sphere()); const newObjectData = { position: new THREE.Vector3(), radius: wordObject.boundingSphere.radius }; switch (effect) { case 'tunnel': const angle = Math.random() * Math.PI * 2; const rMin = parseFloat(tunnelRadiusMin.value); const rMax = parseFloat(tunnelRadiusMax.value); const radius = rMin + Math.random() * Math.max(0, rMax - rMin); const depth = parseFloat(tunnelDepth.value); newObjectData.position.set(Math.cos(angle) * radius, Math.sin(angle) * radius, (Math.random() - 0.5) * depth); break; case 'spiral': const params = { arms: parseInt(spiralArms.value), turns: parseFloat(spiralTurns.value), radiusStart: parseFloat(spiralRadiusStart.value), radiusEnd: parseFloat(spiralRadiusEnd.value), depth: parseFloat(spiralDepth.value), jitter: parseFloat(spiralJitter.value) }; newObjectData.position.copy(getSpiralPosition(i, wordCount, params)); break; case 'zoom': const step = parseFloat(zoomStep.value); newObjectData.position.set((Math.random() - 0.5) * 5, (Math.random() - 0.5) * 5, -i * step); break; case 'sphere': const phi = Math.acos(2 * Math.random() - 1); const theta = Math.random() * Math.PI * 2; const r = parseFloat(sphereRadius.value); newObjectData.position.set(Math.sin(phi) * Math.cos(theta) * r, Math.sin(phi) * Math.sin(theta) * r, Math.cos(phi) * r); break; default: const spread = parseFloat(staticSpread.value); newObjectData.position.set((Math.random() - 0.5) * spread, (Math.random() - 0.5) * spread, (Math.random() - 0.5) * spread); } if (scaleStepVal !== 1) newObjectData.radius *= Math.pow(scaleStepVal, i); if (!isColliding(newObjectData, placedObjects)) { placedObjects.push(newObjectData); wordObject.position.copy(newObjectData.position); wordObject.rotation.set(Math.random() * Math.PI + rotStep.x * i, Math.random() * Math.PI + rotStep.y * i, Math.random() * Math.PI + rotStep.z * i); if (scaleStepVal !== 1) wordObject.scale.multiplyScalar(Math.pow(scaleStepVal, i)); sliceGroup.add(wordObject); break; } } } }
...
function getAllSettings() { const settings = { mode: modeSelect.value, reflections: parseInt(reflectionsInput.value), reflectionsEnabled: reflectionsEnabled.checked, objectCount: parseInt(objectCountInput.value), layout: effectSelect.value, backgroundColor: bgColorPicker.value, selectedShapes: getSelectedShapes(), spiral: { arms: parseInt(spiralArms.value), turns: parseFloat(spiralTurns.value), radiusStart: parseFloat(spiralRadiusStart.value), radiusEnd: parseFloat(spiralRadiusEnd.value), depth: parseFloat(spiralDepth.value), jitter: parseFloat(spiralJitter.value) }, tunnel: { radiusMin: parseFloat(tunnelRadiusMin.value), radiusMax: parseFloat(tunnelRadiusMax.value), depth: parseFloat(tunnelDepth.value) }, zoom: { step: parseFloat(zoomStep.value) }, sphere: { radius: parseFloat(sphereRadius.value) }, static: { spread: parseFloat(staticSpread.value) }, stepTransform: { rotateX: parseFloat(stepRotateX.value), rotateY: parseFloat(stepRotateY.value), rotateZ: parseFloat(stepRotateZ.value), scale: parseFloat(stepScale.value) }, geomSphere: { radius: parseFloat(sphereRadiusGeom.value) }, geomBox: { width: parseFloat(boxWidth.value), height: parseFloat(boxHeight.value), depth: parseFloat(boxDepth.value) }, geomCone: { radius: parseFloat(coneRadius.value), height: parseFloat(coneHeight.value) }, geomCylinder: { radiusTop: parseFloat(cylinderRadiusTop.value), radiusBottom: parseFloat(cylinderRadiusBottom.value), height: parseFloat(cylinderHeight.value) }, geomTorus: { radius: parseFloat(torusRadius.value), tube: parseFloat(torusTube.value) }, textMessage: textInput.value, font: fontSelect.value, reflective: reflectCheckbox.checked, transparent: transparentCheckbox.checked, opacity: parseFloat(opacitySlider.value), eyeSep: parseFloat(eyeSepSlider.value), focalDist: parseFloat(focusSlider.value), fov: parseFloat(fovSlider.value), crossView: eyesSwapped, animate: animateCheckbox.checked, speed: parseFloat(speedSlider.value), viewSize: parseInt(sizeInput.value) }; return settings; }
...
function applySettings(settings) { try { modeSelect.value = settings.mode; reflectionsInput.value = settings.reflections; reflectionsEnabled.checked = settings.reflectionsEnabled; objectCountInput.value = settings.objectCount; effectSelect.value = settings.layout; bgColorPicker.value = settings.backgroundColor; document.querySelectorAll('.shape-checkbox').forEach(cb => { cb.checked = settings.selectedShapes.includes(cb.value); }); if (settings.spiral) { spiralArms.value = settings.spiral.arms; spiralTurns.value = settings.spiral.turns; spiralRadiusStart.value = settings.spiral.radiusStart; spiralRadiusEnd.value = settings.spiral.radiusEnd; spiralDepth.value = settings.spiral.depth; spiralJitter.value = settings.spiral.jitter; } if (settings.tunnel) { tunnelRadiusMin.value = settings.tunnel.radiusMin; tunnelRadiusMax.value = settings.tunnel.radiusMax; tunnelDepth.value = settings.tunnel.depth; } if (settings.zoom) { zoomStep.value = settings.zoom.step; } if (settings.sphere) { sphereRadius.value = settings.sphere.radius; } if (settings.static) { staticSpread.value = settings.static.spread; } if (settings.stepTransform) { stepRotateX.value = settings.stepTransform.rotateX; stepRotateY.value = settings.stepTransform.rotateY; stepRotateZ.value = settings.stepTransform.rotateZ; stepScale.value = settings.stepTransform.scale; } if(settings.geomSphere) sphereRadiusGeom.value = settings.geomSphere.radius; if(settings.geomBox) { boxWidth.value = settings.geomBox.width; boxHeight.value = settings.geomBox.height; boxDepth.value = settings.geomBox.depth; } if(settings.geomCone) { coneRadius.value = settings.geomCone.radius; coneHeight.value = settings.geomCone.height; } if(settings.geomCylinder) { cylinderRadiusTop.value = settings.geomCylinder.radiusTop; cylinderRadiusBottom.value = settings.geomCylinder.radiusBottom; cylinderHeight.value = settings.geomCylinder.height; } if(settings.geomTorus) { torusRadius.value = settings.geomTorus.radius; torusTube.value = settings.geomTorus.tube; } textInput.value = settings.textMessage; fontSelect.value = settings.font; reflectCheckbox.checked = settings.reflective; transparentCheckbox.checked = settings.transparent; opacitySlider.value = settings.opacity; eyeSepSlider.value = settings.eyeSep; focusSlider.value = settings.focalDist; fovSlider.value = settings.fov; eyesSwapped = settings.crossView; animateCheckbox.checked = settings.animate; speedSlider.value = settings.speed; sizeInput.value = settings.viewSize; loadFont(fontSelect.value); updateLayoutParamsVisibility(); updateGeomParamsVisibility(); isAnimating = animateCheckbox.checked; const size = parseInt(sizeInput.value); container.style.width = `${size * 2}px`; container.style.height = `${size}px`; stereoCamera.eyeSep = eyeSepSlider.value / 200; camera.focus = parseFloat(focusSlider.value); camera.fov = parseInt(fovSlider.value); camera.updateProjectionMatrix(); regenerate(); } catch (error) { console.error("Error applying settings:", error); alert("Could not load the settings file. It might be invalid."); } }
...
function setupEventListeners() {
    ...
    resizeButton.addEventListener('click', () => { const size = parseInt(sizeInput.value); container.style.width = `${size * 2}px`; container.style.height = `${size}px`; });
